...
local classColor = {
	...
	{ r = RAID_CLASS_COLORS["DEMONHUNTER"].r, g = RAID_CLASS_COLORS["DEMONHUNTER"].g, b =RAID_CLASS_COLORS["DEMONHUNTER"].b },
	...
	{ r = RAID_CLASS_COLORS["MONK"].r, g = RAID_CLASS_COLORS["MONK"].g, b = RAID_CLASS_COLORS["MONK"].b },
...
local className = {
	[1] = GetClassInfo(6),		-- Death Knight
	[2] = L["Demon Hunter"],	-- Demon Hunter : Legion isn't out yet!
	[3] = GetClassInfo(11),		-- Druid
	[4] = GetClassInfo(3),		-- Hunter
	[5] = GetClassInfo(8),		-- Mage
	[6] = GetClassInfo(10),		-- Monk
	[7] = GetClassInfo(2),		-- Paladin
	[8] = GetClassInfo(5),		-- Priest
	[9] = GetClassInfo(4),		-- Rogue
	[10] = GetClassInfo(7),		-- Shaman
	[11] = GetClassInfo(9),		-- Warlock
	[12] = GetClassInfo(1),		-- Warrior
}
...
local lootColor = {
	...
	{ r = ITEM_QUALITY_COLORS[7].r, g = ITEM_QUALITY_COLORS[7].g, b = ITEM_QUALITY_COLORS[7].b },
}
local lootName = {
	ITEM_QUALITY0_DESC,
	ITEM_QUALITY1_DESC,
...
-- Faction Colors
local factionColor = {
	{ r = PLAYER_FACTION_COLORS[1].r, g = PLAYER_FACTION_COLORS[1].g, b = PLAYER_FACTION_COLORS[1].b },
	{ r = PLAYER_FACTION_COLORS[0].r, g = PLAYER_FACTION_COLORS[0].g, b = PLAYER_FACTION_COLORS[0].b },
}
...
function me:Create_DialogFrame()
	-- Main Dialog Frame
	me.ui = CreateFrame("Frame", myName, ColorPickerFrame, BackdropTemplateMixin and "BackdropTemplate")
...
function me:Create_Toolbar()
	...
	for i = 1, 5 do
		...
		me.ui[key] = CreateFrame("Frame", myName..key, me.ui, BackdropTemplateMixin and "BackdropTemplate")
...
function me:Create_ColorBlock()
	...
	-- Hex Code Editbox -------------------------------------------------------
	me.ui.color["hex"] = CreateFrame("Editbox", myName.."colorhexeditbox", me.ui.color, BackdropTemplateMixin and "BackdropTemplate")
...
function me:Create_HSB()
	...
	for i = 1, 6 do
		...
		me.ui.hsb.hue[key]:SetColorTexture(1.0, 1.0, 1.0, 1.0)
...
function me:Create_BasicFrame(parent, name, width, height, br, bg, bb, ba)
	parent[name] = CreateFrame("Frame", myName..name, parent, BackdropTemplateMixin and "BackdropTemplate")
...
function me:Create_Gradient(parent, r,g,b, rr,gg,bb)
	...
	gradient:SetColorTexture(1.0, 1.0, 1.0, 1.0)	-- For some reason, gradient doesn't work until texture is a solid color
...
function me:Create_ValueBox(parent, suffixMark)
	...
	parent["editbox"] = CreateFrame("Editbox", parent:GetName().."Editbox", parent, BackdropTemplateMixin and "BackdropTemplate")
...
function me:Create_FauxSlider(parent, minValue, maxValue)
	...
	parent["slider"] = CreateFrame("Frame", parent:GetName().."Slider", parent, BackdropTemplateMixin and "BackdropTemplate")
